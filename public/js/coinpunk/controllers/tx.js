...
coinpunk.controllers.Tx.prototype.create = function() {
  ...
  if(address == '')
    ...
    try {
      ...
      errors.push('The provided chancoin address is not valid.');
    }
  }
  ...
  for(var i=0; i<myAddresses.length;i++) {
    if(myAddresses[i].address == address)
      errors.push('You cannot send to your own chancoin wallet.');
  }
  ...
  if(amount == '' || parseFloat(amount) == 0)
    ...
    errors.push('Cannot spend more chancoins than you currently have.');
  }
  ...
  self.saveWallet({override: true, address: changeAddress}, function(response) {
    if(response.result == 'error' && response.messages[0] == 'Invalid session key') {
      ...
      $.post('/api/tx/send', {tx: rawtx}, function(resp) {
        coinpunk.database.setSuccessMessage("Sent "+amount+" CHAN to "+address+".");
...
coinpunk.controllers.Tx.prototype.calculateFee = function() {
  ...
  $('#fee').text(coinpunk.wallet.calculateFee(amount, address, changeAddress)+' CHAN');
...
coinpunk.controllers.Tx.prototype.scanQR = function(event) {
  ...
  qrcode.callback = function(result) {
    ...
    if(uri.protocol() != 'chancoin')
      return errorsDiv.removeClass('hidden').text('Not a valid Chancoin QR code.');
    
    var address = uri.path();
    if(!address || address == '')
      return errorsDiv.removeClass('hidden').text('No Chancoin address found in QR code.');
